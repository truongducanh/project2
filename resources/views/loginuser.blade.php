@extends('layouts.app', ['class' => 'login-page', 'page' => _('Login Page'), 'contentClass' => 'login-page', 'pageSlug' => ''])
...
@section('content')
    ...
    <div class="col-lg-4 col-md-6 ml-auto mr-auto">
        <form class="form" method="post" action="{{ route('login-user-process') }}">
            @csrf

            <div class="card card-login card-white">
                <div class="card-header">
                    <img src="{{ asset('white') }}/img/card-primary.png" alt="">
                    <h1 class="card-title">{{ _('Log in user') }}</h1>
                </div>
                <div class="card-body">
                    ...
                    <div class="input-group{{ $errors->has('password') ? ' has-danger' : '' }}">
                        ...
                        @if (Session::exists('error'))
                            <span class="invalid-feedback" style="display:block !important" role="alert">These credentials do not match our records.</span>
                        @endif
                    </div>
                </div>
                <div class="card-footer">
                    ...
                    <div class="pull-right">
                        <h6>
                            <a href="" class="link footer-link">{{ _('Forgot password?') }}</a>
                        </h6>
                    </div>
                </div>
            </div>
